...
function parseCss(array &$ast, array $options = []): array
{
        ...
        if ($node['name'] === '@import') {
            ...
            if ($matches) {
                ...
                if ($importPath === 'tailwindcss') {
                    ...
                    // Wrap preflight in @layer base so it has lower cascade
                    // priority than unlayered utilities.
                    $preflightLayer = atRule('@layer', 'base', $preflightAst);
                    ...
                    // Combine: theme + @layer base { preflight } + utilities
                    $fullContent = array_merge($themeAst, [$preflightLayer], [$utilitiesNode]);
    ...
    if (!empty($plugins)) {
        ...
        // Inject plugin addBase() styles into @layer base.
        // This ensures they have lower cascade priority than unlayered
        // Tailwind utilities, matching the standard Tailwind CSS v4 layer
        // model.  Both the preflight reset and plugin base styles share
        // @layer base, so specificity applies between them; utilities are
        // unlayered and always win.
        $pluginBaseStyles = $api->getBaseStyles();
        if (!empty($pluginBaseStyles)) {
            $ast = array_merge($ast, [atRule('@layer', 'base', pluginBaseStylesToAst($pluginBaseStyles))]);
        }
    }
...
/**
 * Convert plugin addBase() style-sets to AST nodes.
 *
 * Builds nodes directly instead of serialising to CSS and re-parsing,
 * which avoids issues with comment tokens inside CSS values
 * (e.g. var(--tw-empty,/*!*/ /*!*\/)).
 *
 * Each style-set is $selector => $declarations where declarations may
 * contain plain string values or one level of nesting:
 *   - '&:pseudo' => [...] resolves the & against the outer selector
 *   - '@media ...' => [...] wraps a nested rule in an at-rule
 *
 * @param array $styleSets The value of PluginAPI::getBaseStyles()
 * @return array AST nodes ready to merge into the main $ast
 */
function pluginBaseStylesToAst(array $styleSets): array
{
    $nodes = [];

    foreach ($styleSets as $styleSet) {
        foreach ($styleSet as $selector => $declarations) {
            $directDecls = [];
            $nested = [];
            ...
            foreach ($declarations as $prop => $value) {
                if (is_array($value)) {
                    ...
                    $directDecls[] = \TailwindPHP\Ast\decl((string) $prop, (string) $value);
                }
            }

            if (!empty($directDecls)) {
                $nodes[] = \TailwindPHP\Ast\styleRule((string) $selector, $directDecls);
            }

            foreach ($nested as $nestedKey => $nestedDecls) {
                $innerDeclNodes = [];
                foreach ($nestedDecls as $p => $v) {
                    if (!is_array($v)) {
                        $innerDeclNodes[] = \TailwindPHP\Ast\decl((string) $p, (string) $v);
                    }
                }
                if (empty($innerDeclNodes)) {
                    continue;
                }
                if (str_starts_with((string) $nestedKey, '@')) {
                    // e.g. '@media (forced-colors: active)'
                    $parts = explode(' ', ltrim((string) $nestedKey, '@'), 2);
                    $atName = '@' . $parts[0];
                    $atParams = $parts[1] ?? '';
                    $innerRule = \TailwindPHP\Ast\styleRule((string) $selector, $innerDeclNodes);
                    $nodes[] = \TailwindPHP\Ast\atRule($atName, $atParams, [$innerRule]);
                } else {
                    // e.g. '&:focus' => resolve & against outer selector
                    $resolved = str_replace('&', (string) $selector, (string) $nestedKey);
                    $nodes[] = \TailwindPHP\Ast\styleRule($resolved, $innerDeclNodes);
                }
            }
        }
    }
