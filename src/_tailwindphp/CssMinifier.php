...
class CssMinifier
{
    ...
    private static function removeWhitespace(string $css): string
    {
        ...
        // Remove space around special characters (excluding : which needs special handling
        // to preserve descendant+pseudo-class selectors like `.prose :where(p)`)
        $css = preg_replace('/\s*([{};,>~+])\s*/', '$1', $css);

        // For : only strip space AFTER (e.g. `color: red` → `color:red`)
        // but NOT before, which would break `.prose :where(p)` → `.prose:where(p)`
        $css = preg_replace('/:\s+/', ':', $css);
