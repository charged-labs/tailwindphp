...
final class FormsPlugin implements PluginInterface
{
     ...
     */
    private function getRules(array $options = []): array
    {
        $gray500 = '#6b7280';
        $blue600 = '#2563eb';

        $borderRadius = $options['border-radius'] ?? '0px';
        $borderColor = $options['border-color'] ?? $gray500;
        $paddingX = $options['padding-x'] ?? $this->spacing['3'];
        $paddingY = $options['padding-y'] ?? $this->spacing['2'];
        $fontSize = $options['font-size'] ?? '1rem';
        $lineHeight = $options['line-height'] ?? '1.5rem';
        $focusRingColor = $options['focus-ring-color'] ?? $blue600;
        $placeholderColor = $options['placeholder-color'] ?? $gray500;
        ...
        return [
            // Text inputs, textareas, selects — uses configurable options.
            [
                'base' => [
                    "[type='text']",
